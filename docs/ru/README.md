...
- **Zero Trust Security & Policy Enforcement (Безопасность и политики):**
  - Обязательная подпись HMAC SHA256 для всех сообщений при наличии `WORKER_TOKEN`.
  - Проверка подписи оркестратора (`orchestrator_signature` / `sig`) входящих задач перед выполнением.
  - Строгое соблюдение политик выполнения задач (`allowed_skills`).
  - Автоматический сбор и отчётность метрик затрат и времени выполнения задач (`costs`).
